...
class TestMoneyCommunity(DispersyTestFunc):
    ...
    def setUp(self):
        ...
        self.dummy1_a.persistent_storage['registered'] = True
        self.dummy1_b = Dummy1Manager(database)
        self.dummy1_b.persistent_storage['registered'] = True
        self.dummy2_b = Dummy2Manager(database)
        self.dummy2_b.persistent_storage['registered'] = True

        self.node_a, self.node_b = yield self.create_nodes(2)

        self.node_a.community.database = database
        self.node_b.community.database = database
    ...
    def introduce_candidates(self):
        ...
        yield self.parse_assert_packets(self.node_b)  # Introduction response + services info
        yield self.parse_assert_packets(self.node_a)  # Services info
        yield deferLater(reactor, 0.1, lambda: None)
    ...
    def test_has_eligable_router(self):
        ...
        """
        yield self.introduce_candidates()
    ...
    def test_rollback(self):
        """
        Test a rollback of a transaction
        """
        self.node_b.community.tx_fee = 500.0
        yield self.introduce_candidates()
